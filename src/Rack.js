...
function Rack() {
  ...
  let handle = (e) => {
    ...
    console.log(new Date()-input3.current.value);
  }
  return (<div>
    ...
    Date   :<input type="date" required  ref={input3} onChange={(e)=>console.log(e.target.value)}/><br/>
      ...
      if (element.display === true && element.flag === true) {
        ...
        return (<fieldset>
          <a href="#" onClick={() => dispatcher(DeleteBook(element.id))} className="deletebutton">Delete</a>
                ...
                <p>Date :{moment().endOf(element.date).fromNow()}</p>
                </fieldset>
          );
      }
